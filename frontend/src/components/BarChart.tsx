    ...
    return (
      <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-2xs space-y-4">
        ...
        <div className="space-y-4">
          {propLabels.map((lbl, i) => (
            <div key={lbl} className="space-y-1.5">
              <span className="text-xs font-semibold text-slate-800 block">{lbl}</span>
                ...
                {propDatasets.map((ds) => {
                  ...
                  return (
                    <div key={ds.name} className="flex items-center gap-3 text-xs font-mono">
                      <div className="h-2 flex-1 rounded-full bg-slate-100 overflow-hidden border border-slate-200/60">
                        <div
                          className="h-full rounded-full transition-all duration-500"
                          style={{
                            width: `${pct}%`,
                            backgroundColor: ds.color || '#059669',
                          }}
                        />
                      </div>
                      <span className="w-20 text-right text-slate-700 font-semibold text-[11px]">
                        {val.toFixed(1)}
                      </span>
                    </div>
                  );
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 pt-3 border-t border-slate-100 text-xs">
          {propDatasets.map((ds) => (
            <div key={ds.name} className="flex items-center gap-1.5 font-mono text-[11px] text-slate-700">
              <span
                className="h-2.5 w-3 rounded-full"
                style={{ backgroundColor: ds.color || '#059669' }}
              />
              <span className="font-sans font-medium text-slate-700">{ds.name}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        {items.map((item, idx) => {
          ...
          return (
            <div key={idx} className="space-y-1.5">
              ...
              <div className="relative h-2.5 w-full rounded-full bg-slate-100 overflow-hidden border border-slate-200/60">
                <div
                  className="h-full rounded-full transition-all duration-500"
