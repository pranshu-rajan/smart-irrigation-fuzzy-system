    ...
    <div className="space-y-8 pb-16">
      ...
      {activeCtrl && (
        <div className="rounded-2xl border border-emerald-200/80 bg-emerald-50/40 p-4 text-xs text-slate-700 flex flex-wrap items-center justify-between gap-4 shadow-2xs">
          <div>
            <span className="font-bold text-emerald-950 uppercase tracking-wider">{activeCtrl.display_name}</span>
          ...
          <div className="flex flex-wrap gap-4 font-mono text-[11px] text-slate-600">
            <span>Inputs: <strong className="text-slate-900">{activeCtrl.inputs.length}</strong></span>
            <span>Outputs: <strong className="text-slate-900">{activeCtrl.outputs.length}</strong></span>
            <span>Rules: <strong className="text-slate-900">{activeCtrl.rule_count}</strong></span>
                ...
                <div className="max-h-36 overflow-y-auto space-y-1.5 font-mono text-[10px] text-slate-700">
                  {evalResult.fired_rules.map((fr) => (
                    <div key={fr.rule_index} className="rounded-lg bg-white border border-emerald-100 p-2 flex justify-between shadow-2xs">
                      <span className="truncate max-w-[180px] font-sans font-medium text-slate-800">R{fr.rule_index + 1}: {fr.consequent}</span>
                      <span className="text-emerald-700 font-bold">μ={fr.firing_strength.toFixed(3)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <tbody className="divide-y divide-slate-100 font-mono text-[11px]">
                {filteredRules.map((r) => (
                  <tr key={r.rule_index} className="hover:bg-emerald-50/40 transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-right text-slate-500">{r.weight.toFixed(1)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
